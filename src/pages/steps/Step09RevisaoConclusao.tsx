...
interface Step09RevisaoConclusaoProps {
...
export function Step09RevisaoConclusao({
  ...
  const itemsChecados = [
    ...
    { label: STEP_LABELS[5], check: (levantamento.epis_epcs_evidencias?.evidencias ?? []).length > 0 },
    { label: STEP_LABELS[6], check: (levantamento.medicoes ?? []).length > 0 },
    { label: STEP_LABELS[7], check: (levantamento.riscos ?? []).length > 0 },
    { label: STEP_LABELS[8], check: !!(conclusao || assinaturaTecnico.nome) },
  ]
